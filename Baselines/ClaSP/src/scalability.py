...
from clasp import extract_clasp_cps, extract_clasp_cps_from_multivariate_ts
...
def effect_of_length():
    time_list = []
    path = os.path.join(data_path, 'synthetic/test0.csv')
...
def effect_of_dimension():
    time_list = []
    data = np.loadtxt(os.path.join(data_path, 'synthetic/test0.csv'), delimiter=',')
    data = data[:,:4]
